...
def ensure_ffmpeg_available(raise_on_missing: bool = True):
    ...
    msg_lines = [
        ...
        " 2) On Streamlit Cloud add a top-level 'packages.txt' file containing 'ffmpeg' so the build installs system ffmpeg/ffprobe. (I've added a packages.txt in this repo.)",
        " 3) Set environment variables FFMPEG_PATH and FFPROBE_PATH to the full paths of ffmpeg/ffprobe.",
        " 4) Install system ffmpeg on the host (apt/yum/brew or include in your Docker image).",
        "\nCurrent detection:",
